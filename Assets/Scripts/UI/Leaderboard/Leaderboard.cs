...
public class Leaderboard : NetworkBehaviour
{
    ...
    private void LeaderboardEntities_OnListChanged(NetworkListEvent<LeaderboardEntityState> changeEvent) {
        ...
        LeaderboardEntityDisplay myDisplay = entityDisplays.FirstOrDefault(x => x.ClientId == NetworkManager.Singleton.LocalClientId);//Gets your player specific display

        if (myDisplay != null) {
            if (myDisplay.transform.GetSiblingIndex() >= entitiesToDisplay) {//On leaderboard
                leaderboardEntityHolder.GetChild(entitiesToDisplay - 1).gameObject.SetActive(false);
                myDisplay.gameObject.SetActive(true);//Always shows yours, not the 8th place
            }
        }
    }
